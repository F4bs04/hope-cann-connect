-- CRITICAL SECURITY FIXES - Phase 1: Create Admin Table and RLS Security
...
-- Insert example admin user if not exists
INSERT INTO public.usuarios (email, senha_hash, tipo_usuario, status)
VALUES ('admin@clinica.com', crypt('admin123', gen_salt('bf')), 'admin', true)
ON CONFLICT (email) DO NOTHING;

-- Insert admin record if not exists
INSERT INTO public.admins (id_usuario, nome, cargo, permissoes)
SELECT u.id, 'Administrador Sistema', 'admin', '["manage_doctors", "view_all_data", "manage_system"]'::json
FROM public.usuarios u
WHERE u.email = 'admin@clinica.com'
AND NOT EXISTS (
  SELECT 1 FROM public.admins a WHERE a.id_usuario = u.id
);
