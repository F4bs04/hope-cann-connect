...
CREATE POLICY IF NOT EXISTS "Admins can view their own data" ON public.admins
FOR SELECT USING (
  id_usuario = (
...
CREATE POLICY IF NOT EXISTS "Medicos can view their consultations" ON public.consultas
FOR SELECT USING (
  id_medico IN (
...
CREATE POLICY IF NOT EXISTS "Pacientes can view their consultations" ON public.consultas
FOR SELECT USING (
  id_paciente IN (
...
CREATE POLICY IF NOT EXISTS "Anyone can view approved medicos" ON public.medicos
FOR SELECT USING (aprovado = true);

CREATE POLICY IF NOT EXISTS "Medicos can view their own profile" ON public.medicos
FOR SELECT USING (
  id_usuario = (
    SELECT id FROM public.usuarios 
    WHERE email = auth.email()
...
CREATE POLICY IF NOT EXISTS "Pacientes can view their own data" ON public.pacientes
FOR SELECT USING (
  id_usuario = (
    SELECT id FROM public.usuarios 
    WHERE email = auth.email()
