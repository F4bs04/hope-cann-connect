-- ========================================
-- ETAPA 2: ÍNDICES DE PERFORMANCE (SEM CONCURRENTLY)
-- ========================================

-- Índices para a nova tabela de pacientes
CREATE INDEX IF NOT EXISTS idx_pacientes_unified_email ON public.pacientes_unified(email);
CREATE INDEX IF NOT EXISTS idx_pacientes_unified_cpf ON public.pacientes_unified(cpf) WHERE cpf IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_pacientes_unified_id_usuario ON public.pacientes_unified(id_usuario) WHERE id_usuario IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_pacientes_unified_data_cadastro ON public.pacientes_unified(data_cadastro);
...
CREATE INDEX IF NOT EXISTS idx_consultas_id_medico_data ON public.consultas(id_medico, data_hora);
...
CREATE INDEX IF NOT EXISTS idx_medicos_aprovado ON public.medicos(aprovado);

-- Índices para horários disponíveis
CREATE INDEX IF NOT EXISTS idx_horarios_disponiveis_medico_dia ON public.horarios_disponiveis(id_medico, dia_semana);

-- ========================================
-- ETAPA 3: TRIGGERS E FUNÇÕES
-- ========================================
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE c.id_medico = p_medico_id
      ...
      AND ABS(EXTRACT(EPOCH FROM (c.data_hora - p_data_hora))) < 1800
  ) INTO v_conflito;
