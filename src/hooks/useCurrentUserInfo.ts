    ...
    const loadUserInfo = async () => {
      try {
        ...
        if (userData.tipo_usuario === 'medico') {
          ...
          // Primeiro, tenta buscar na tabela pacientes
          ...
          if (pacienteData) {
            ...
            // Se não encontrar, tenta buscar na tabela pacientes_app
            const email = session.user.email;
            const { data: pacienteAppData, error: pacienteAppError } = await supabase
              .from('pacientes_app')
              .select('id')
              .eq('email', email)
              .maybeSingle();
            ...
            if (pacienteAppData) {
              info.pacienteId = pacienteAppData.id;
            }
          }
        }
