  ...
  return <header className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-3' : 'bg-transparent py-4'}`}>
      <div className="hopecann-container flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          {session ? userType === 'medico' ? <Link to="/area-medico" className="bg-hopecann-green hover:bg-hopecann-teal text-white font-medium py-2.5 px-6 rounded-full transition-all shadow-sm flex items-center gap-2">
                <Stethoscope size={18} />
                Área do Médico
              </Link> : <Link to="/area-paciente" className="bg-hopecann-green hover:bg-hopecann-teal text-white font-medium py-2.5 px-6 rounded-full transition-all shadow-sm flex items-center gap-2">
                <User size={18} />
                Área do Paciente
              </Link> : <Link to="/login" className="bg-hopecann-green hover:bg-hopecann-teal text-white font-medium py-2.5 px-6 rounded-full transition-all shadow-sm flex items-center gap-2">
              <LogIn size={18} />
              Login
            </Link>}
        </nav>
      ...
      {isMenuOpen && <div className="md:hidden bg-white shadow-lg py-6 absolute top-full left-0 right-0 animate-fade-in">
          <div className="hopecann-container flex flex-col space-y-6">
            ...
            {session ? userType === 'medico' ? <Link to="/area-medico" onClick={() => setIsMenuOpen(false)} className="bg-hopecann-green hover:bg-hopecann-teal text-white font-medium py-3 px-6 rounded-full transition-all w-full text-center flex items-center justify-center gap-2">
                  <Stethoscope size={18} />
                  Área do Médico
                </Link> : <Link to="/area-paciente" onClick={() => setIsMenuOpen(false)} className="bg-hopecann-green hover:bg-hopecann-teal text-white font-medium py-3 px-6 rounded-full transition-all w-full text-center flex items-center justify-center gap-2">
                  <User size={18} />
                  Área do Paciente
                </Link> : <Link to="/login" onClick={() => setIsMenuOpen(false)} className="bg-hopecann-green hover:bg-hopecann-teal text-white font-medium py-3 px-6 rounded-full transition-all w-full text-center flex items-center justify-center gap-2">
                <LogIn size={18} />
                Login
              </Link>}
          </div>
