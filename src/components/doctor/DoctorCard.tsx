...
const DoctorCard = ({ doctor, onSelect, isSelected = false }: DoctorCardProps) => {
  return (
    <div
      key={doctor.id}
      className={`p-4 border rounded-lg cursor-pointer transition-all duration-300 ${isSelected 
        ? 'border-hopecann-teal border-2 shadow-lg shadow-hopecann-teal/20 animate-pulse-border' 
