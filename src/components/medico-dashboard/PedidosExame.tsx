...
import { AlertTriangle, FileText, Check, Save, BookmarkPlus, Search, Eye } from 'lucide-react';
...
import ExamePreview from '@/components/medico/ExamePreview';
...
const PedidosExame: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const medicoNome = userInfo?.medicoNome || localStorage.getItem("medicoNome") || "Médico Responsável";
      const medicoCrm = userInfo?.medicoCrm || localStorage.getItem("medicoCrm") || "12345";
      const medicoEspecialidade = userInfo?.medicoEspecialidade || localStorage.getItem("medicoEspecialidade") || "Clínico Geral";
  ...
  const handlePreview = () => {
    if (!pacienteId) {
      toast({
        ...
        description: "Por favor, selecione um paciente para visualizar o pedido de exame",
      });
      return;
    }
    
    if (activeTab === 'formulario' && (!nomeExame || !justificativa)) {
      toast({
        variant: "destructive",
        title: "Campos obrigatórios",
        description: "Por favor, preencha o nome do exame e a justificativa",
      });
      return;
    }
    ...
    if (activeTab === 'pdf' && !pdfFilePath) {
      toast({
        ...
        description: "Por favor, faça upload do arquivo PDF do pedido de exame",
      });
      return;
    }
    
    // Se tudo estiver preenchido, mostra o preview
    setShowPreview(true);
  };
  ...
  const filteredTemplates = templates.filter(template => 
    ...
    (template.justificativa?.toLowerCase() || '').includes(templateSearchQuery.toLowerCase())
  );
  
  // Ordenar por uso mais recente
  const sortedTemplates = [...filteredTemplates].sort((a, b) => 
    new Date(b.ultima_utilizacao || 0).getTime() - new Date(a.ultima_utilizacao || 0).getTime()
  );
  
  // Atualizar o paciente selecionado quando o ID mudar
  useEffect(() => {
    if (pacienteId) {
  ...
  // Já definimos filteredTemplates acima
  ...
  // Se estiver mostrando o preview, renderize o componente ExamePreview
  if (showPreview && pacienteId) {
    const exameData = {
      id_paciente: parseInt(pacienteId),
      paciente: pacienteSelecionado ? {
        nome: pacienteSelecionado.nome,
        idade: pacienteSelecionado.idade || 0,
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-between pt-4">
                  <div className="flex gap-2 flex-1">
                    <Button
                      ...
                      disabled={!nomeExame || !justificativa}
                    ...
                    <Button 
                      type="button" 
                      className="bg-hopecann-teal hover:bg-hopecann-teal/90 text-white flex-1"
                      disabled={(activeTab === 'pdf' && !pdfFilePath) || (activeTab === 'formulario' && (!pacienteId || !nomeExame || !justificativa))}
