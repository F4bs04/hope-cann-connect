...
import { Calendar as CalendarIcon, Download, Printer, Check, Eye } from 'lucide-react';
...
import AtestadoPreview from '@/components/medico/AtestadoPreview';
...
const Atestados: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Carregar informações do médico do localStorage
    const medicoNome = localStorage.getItem("medicoNome") || "Médico Responsável";
    const medicoCrm = localStorage.getItem("medicoCrm") || "12345";
    const medicoEspecialidade = localStorage.getItem("medicoEspecialidade") || "Clínico Geral";
  ...
  const handlePreview = () => {
    if (!pacienteId) {
      toast({
        ...
        description: "Por favor, selecione um paciente para visualizar o atestado",
      });
      return;
    }

    if (activeTab === 'formulario' && (!tempoAfastamento || !justificativa)) {
      toast({
        variant: "destructive",
        title: "Campos obrigatórios",
        description: "Por favor, preencha o tempo de afastamento e a justificativa",
      });
      return;
    }
    ...
    if (activeTab === 'pdf' && !pdfFilePath) {
      toast({
        ...
        description: "Por favor, faça upload do arquivo PDF do atestado",
      });
      return;
    }
  ...
  // Se estiver mostrando o preview, renderize o componente AtestadoPreview
  if (showPreview && pacienteId) {
    ...
    const atestadoData = {
      ...
      data_consulta: dataConsulta || new Date(),
      tempo_afastamento: tempoAfastamento,
      unidade: unidade,
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end pt-4 gap-4">
                  <Button
                    ...
                    disabled={(activeTab === 'pdf' && !pdfFilePath) || (activeTab === 'formulario' && (!pacienteId || !tempoAfastamento || !justificativa))}
                    onClick={handlePreview}
                  >
                    <Eye className="h-4 w-4 mr-2" /> Visualizar Atestado
                  </Button>
                  <Button
                    type="submit"
