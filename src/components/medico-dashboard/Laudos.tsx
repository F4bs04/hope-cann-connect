...
import { getPacientes, createLaudo, getPacienteById } from '@/services/supabaseService';
import PdfUpload from '@/components/ui/pdf-upload';
import LaudoPreview from '@/components/medico/LaudoPreview';
...
const Laudos: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const nome = localStorage.getItem("userName") || "Médico HopeCann";
    const crm = localStorage.getItem("userCRM") || "12345";
    const especialidade = localStorage.getItem("userEspecialidade") || "Medicina Canábica";
  ...
  const handleChangePaciente = async (id: string) => {
    setPacienteId(id);
    if (id) {
      try {
  ...
  const handlePreview = async () => {
    console.log('handlePreview chamado');
    ...
    if (!pacienteId) {
      toast({
        ...
        description: "Por favor, selecione um paciente para visualizar o laudo",
      });
      return;
    }
    ...
    // Buscar dados do paciente se ainda não tiver
    if (!pacienteSelecionado && pacienteId) {
      try {
        const paciente = await getPacienteById(parseInt(pacienteId));
        if (paciente) {
          setPacienteSelecionado(paciente);
          // Esperar o estado ser atualizado
          setTimeout(() => {
            setShowPreview(true);
          }, 100);
          return;
    ...
    // Se já temos o paciente, mostrar o preview
    setShowPreview(true);
  };
  ...
  const validarFormulario = () => {
    ...
    if (!assinado) {
      toast({
        ...
        description: "Por favor, assine digitalmente o laudo antes de gerar",
      });
      return false;
    }
  ...
  // Mostra o preview do laudo quando showPreview é true e temos um paciente selecionado
  if (showPreview && pacienteId) {
    // Se não temos dados do paciente ainda, tente usar informações básicas
    const pacienteInfo = pacienteSelecionado || {
    ...
    const laudoData = {
      ...
      paciente: {
        ...
        idade: pacienteInfo.idade || 0,
        genero: pacienteInfo.genero || "",
        endereco: pacienteInfo.endereco || ""
      },
      tipo_laudo: tipoLaudo,
      objetivo: objetivo,
      descricao: descricao,
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-between pt-4">
              <Button 
                type="button" 
                className="bg-hopecann-teal hover:bg-hopecann-teal/90 flex-1 mr-2"
                disabled={!pacienteId || (activeTab === 'formulario' && (!tipoLaudo || !objetivo || !descricao || !conclusao))}
                onClick={() => {
                  console.log('Botão de visualizar clicado');
                  handlePreview();
                }}
              >
                <Eye className="h-4 w-4 mr-2" /> Visualizar Laudo
              </Button>
